...
${lessonsContext}
          ...
          const lessonOutput = parsePartialJson(
            streamOutput,
            ALL,
          ) as LessonSchemaType;

          // Add item ids to each chunk item
          addItemIds(lessonOutput);
  ...
  tool(
    async (
      ...
      try {
        ...
        const { lesson: lessonNode } = await getLesson(lessonId, userId);
        if (!lessonNode)
...
/**
 * Add item ids to each chunk item. The id generated by the ai are not v4 uuid, so we need to generate new ones.
 */
function addItemIds(lesson: LessonSchemaType) {
