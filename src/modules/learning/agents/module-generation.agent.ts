...
export class ModuleGenerationAgent {
  ...
  async generateModuleStructure(
    ...
    try {
      // Define the output schema matching ModuleGenerationResult
      ...
      // Create the prompt template (no format_instructions needed with withStructuredOutput)
      const systemTemplate = `You are an expert educational content designer specialized in creating structured learning modules.
Your task is to analyze the user's prompt and generate a well-organized learning module structure.
...
- Ensure all content matches the requested difficulty level: {difficulty}.`;
      ...
      // Use withStructuredOutput for automatic structured response parsing
      const result = await this.llm
        .withStructuredOutput(moduleStructureSchema)
        .invoke(messages);

      return result as ModuleGenerationResult;
    } catch (error) {
      this.logger.error(
