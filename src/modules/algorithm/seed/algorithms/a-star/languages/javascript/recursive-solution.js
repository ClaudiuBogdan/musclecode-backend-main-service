...
function aStarSearch(grid, start, goal) {
  ...
  let openList = [new Node(start[0], start[1], 0, heuristic(start[0], start[1]), null)];

  function recursiveAStar(openList) {
    ...
    return recursiveAStar(openList);
  }

  const result = recursiveAStar(openList);
  if (!result) return [];
  let path = [];
  let curr = result;
  while (curr) {
